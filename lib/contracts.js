...
// schedule_rows cascades via ON DELETE CASCADE in the schema.
...
async function upsertContract(contract) {
  ...
  if (!Number.isInteger(Number(contract.termMonths)) || Number(contract.termMonths) <= 0) throw new Error(`contract ${contract.id}: termMonths must be a positive integer`);
  for (const field of ['principal', 'interestRate', 'totalReceivable']) {
    if (!Number.isFinite(Number(contract[field])) || Number(contract[field]) < 0) throw new Error(`contract ${contract.id}: ${field} must be a non-negative number`);
  }

  const schedule = Array.isArray(contract.schedule) ? contract.schedule : [];
  for (const row of schedule) {
    if (!row.dueDate) continue;
    if (!Number.isInteger(Number(row.sequence)) || Number(row.sequence) <= 0) throw new Error(`contract ${contract.id}: invalid schedule sequence`);
    if (!Number.isFinite(Number(row.scheduled)) || Number(row.scheduled) < 0) throw new Error(`contract ${contract.id}: invalid scheduled amount`);
    if (!Number.isFinite(Number(row.paid)) || Number(row.paid) < 0) throw new Error(`contract ${contract.id}: invalid paid amount`);
  }
  ...
  queries.push(sql`DELETE FROM schedule_rows WHERE contract_id = ${contract.id}`);
  for (const row of schedule) {
    if (!row.dueDate) continue;
    queries.push(sql`
...
module.exports = { fetchAllContracts, upsertContract, deleteContract, rowToContract };
