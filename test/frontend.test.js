...
test('portfolio status categories expand contract details and the profile stays static', () => {
  assert.doesNotMatch(html, /profileStatusToggle|profile-status-popover|toggleProfileStatus/);
  assert.match(html, /function toggleRiskStatus\(status\)/);
  assert.match(html, /class="risk-category/);
  assert.match(html, /class="risk-detail-item/);
